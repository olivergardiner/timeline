...
public abstract class TimeBar {
	...
	protected void applyPrecision() {
		...
		case QUARTERS:
			...
			int quarter = barStart.get(Calendar.MONTH) / 3;
	...
	protected void createPeriods() {
		...
		while (calendar.before(barEnd)) {
			...
			case MONTHS:
				...
				break;
			case QUARTERS:
				period.setLabel("Q" + (calendar.get(Calendar.MONTH) / 3 + 1) + " " + calendar.get(Calendar.YEAR));
			...
			case WEEKS:
				...
				break;
			case YEARS:
				period.setLabel(Integer.toString(calendar.get(Calendar.YEAR)));
